...
Original file is located at
    https://colab.research.google.com/drive/1H1jw-nMv0_oXOdCopfpmxTqEpIFiDqx2
"""
...
## Answer:
1: overfitting 문제 보여줌. batch norm에는 normalization 효과가 있으므로 더 나은 결과를 얻을 수 있음

2: vanishing gradient 문제를 보여줌. 반면, batch norm은 weigh initialization에 덜 민감함

3: gradient explode problem을 보여줌. batch norm에서는 이런 문제 발생하지 않음

batch norm을 이용하면 normalization을 하여 튀는 값이 줄어들기 때문에 gradient vanishing/exploding problem을 방지할 수 있음. 또, normalization 덕분에 overfitting도 감소함
...
## Answer:
batch size는 batch normalization의 성능에 직접적으로 영향을 미친다. batch가 작을수록 성능이 더 안 좋아진다. batch size가 작을 경우 noise가 발생 가능하다.
...
## Answer:
1: 틀림. 앞에서 5-layer network가 있었고, 이는 deep network라 할 수 있다. using layer normalization in a very deep network는 올바르게 작동할 것이다.

2: 맞음. feature의 dimension이 작으면 layer normalization에 영향을 미치는데, hidden size가 작을수록 noise가 더 많아진다.

3: 맞음. underfitting을 유발할 수 있다.
